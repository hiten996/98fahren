...
_filterthumbnail: function(variant){
  console.log('filterthumbnail', variant);
...
$('[data-thumbnail-color]').hide();
    ...
    var thumbnail_selected = '[data-thumbnail-color="' + selected_color + '"]';
    $(thumbnail_selected).show();
  }
  else
  {
  ...
  theme.Product = (function() {
    ...
    function Product(container) {
      ...
      this.selectors = {
        ...
        sizeLabel: '[data-variant-size-label]',
    ...
    Product.prototype = Object.assign({}, Product.prototype, {
      ...
      initVariants: function() {
        ...
        if (swatches.length) {
          ...
          var swatches = this.container.querySelectorAll(this.selectors.variantSizeSwatch);
        if (swatches.length) {
          swatches.forEach(swatch => {
            swatch.addEventListener('change', function(evt) {
              var size = swatch.dataset.sizeName;
              var index = swatch.dataset.sizeIndex;
              this.updateSizeName(size, index);
            }.bind(this))
          });
        }
      ...
      updateSizeName: function(size, index) {
        // Updates on radio button change, not variant.js
        this.container.querySelector(this.selectors.sizeLabel + `[data-index="${index}"`).textContent = size;
      },
...
$(document).ready(function() {
   const videoText = document.querySelector(".video_with_text_main");
    const videoTextVideoAll = document.querySelectorAll(".video_with_text_video .slide_video");
    const videoTextImageAll = document.querySelectorAll(".video_with_text_video .video_bg .video_bg_img");
   
    videoText.addEventListener('click', (e) => {
      let htmlShowBlock = e.target.closest('.video_btn_content');
      let id = e.target.closest('.video_btn_content').getAttribute('data-video');
      const videoSelf = document.querySelector(`.slide_video[data-video="${id}"]`);
      const imageSelf = document.querySelector(`.video_bg_img img[data-video="${id}"]`);
      const videoslfcntn = document.querySelector(`.vdotitlecntn`);
      const icon = document.querySelector(`.video_bg [data-video="${id}"] .flipIcon`);

      videoTextImageAll.forEach((img) => img.style.display = 'none');
      videoTextVideoAll.forEach((video) => {
        videoSelf.style.display = 'block';
        videoSelf.pause();
      });
      

      if(htmlShowBlock.classList.contains('pause-state')){
           
      imageSelf.style.display = "none"; 
        videoslfcntn.style.display = "none"; 
       videoSelf.style.display = "block";
       videoSelf.play(); 
   icon.src = `https://cdn.shopify.com/s/files/1/0526/2688/2746/files/pause-circle-outline.svg?v=1681465342`;
        htmlShowBlock.classList.remove('pause-state');
  htmlShowBlock.classList.add('play-state');
        icon.classList.add('pausestate');
         }else{
        imageSelf.style.display = "none";
        videoslfcntn.style.display = "none"; 
       videoSelf.style.display = "none";
       videoSelf.pause(); 
           icon.src = `https://cdn.shopify.com/s/files/1/0526/2688/2746/files/play-circle-outline.svg?v=1681471140`;
        htmlShowBlock.classList.add('pause-state');
        htmlShowBlock.classList.remove('play-state');
        icon.classList.remove('pausestate');
         }
  ...
  $(document).ready(function(){
    $('.centimeter').click(function(){
      $('.centimeter').addClass('tabsactv');
      $('.inc').removeClass('tabsactv');
      $('.cntimterimgs').addClass('desktopshow');
      $('.incesimg').removeClass('desktopshow');


      $('.mobcntimterimgs').addClass('mobileshow');
      $('.mobincesimg').removeClass('mobileshow');
      
      $('.incesimg').addClass('hide');
       $('.cntimterimgs').removeClass('hide');

      $('.mobincesimg').addClass('hide');
       $('.mobcntimterimgs').removeClass('hide');
      
    });


    $('.inc').click(function(){
         $('.inc').addClass('tabsactv');
      $('.centimeter').removeClass('tabsactv');
      $('.incesimg').addClass('desktopshow');
      $('.cntimterimgs').removeClass('desktopshow');

      $('.mobincesimg').addClass('mobileshow');
      $('.mobcntimterimgs').removeClass('mobileshow');
      
      $('.mobcntimterimgs').addClass('hide');
      $('.incesimg').removeClass('hide');


$('.cntimterimgs').addClass('hide');
      $('.mobincesimg').removeClass('hide');
...
$(".sizebtn").click(function() {
    $('html, body').animate({
        scrollTop: $(".sizesection").offset().top
    }, 2000);
});
